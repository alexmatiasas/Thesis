...
La estadística de Tsallis generaliza la entropía de \gls{bg} mediante la introducción de un parámetro no extensivo \( q \), definido por:
...
de donde se observa claramente que cuando $q=1$ se regresa a la estadística de \gls{bg} %[ref Tsallis statistics].
...
% Este capítulo se divide en varias secciones. En la primera, se presenta la entropía de Tsallis y su relación con la entropía de \gls{bg}. Posteriormente, se discuten las propiedades clave de la estadística de Tsallis, incluyendo su aplicación a sistemas no extensivos. Finalmente, se explorará cómo esta estadística puede aplicarse al estudio de hadrones y al confinamiento de quarks, proporcionando una descripción más general de sistemas complejos.
